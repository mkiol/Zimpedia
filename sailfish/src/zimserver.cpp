...
QList<SearchResult> ZimServer::search(const QString &value)
{
    ...
    if (getLoaded()) {
        ...
        if (!t.isEmpty()) {
            try {
                auto appendResult = [&result, &urls, this](auto begin, auto end){
                    auto it = begin;
                    for (int i = 0; it != end && i < maxSearch; ++it, ++i) {
                        if (it->getLibraryMimeType() != std::numeric_limits<uint16_t>::max() &&
                            QString::fromStdString(it->getMimeType()).startsWith("text", Qt::CaseInsensitive)) {
                            auto url = getLocalUrl(ZimServer::stringStdToQ(it->getLongUrl()));
                            if (!urls.contains(url)) {
                                result << SearchResult{ZimServer::stringStdToQ(it->getTitle()), url};
                                urls << url;
                            }
                        }
                    }
                ...
                if (result.isEmpty()) {
                    ...
                    t[0] = t[0].toUpper();
                    auto itu = zimfile->findByTitle('A', std::string{t.toUtf8().constData()});
                    appendResult(itu, zimfile->end());

                    // first letter is lower case
                    t[0] = t[0].toLower();
                    auto itl = zimfile->findByTitle('A', std::string{t.toUtf8().constData()});
